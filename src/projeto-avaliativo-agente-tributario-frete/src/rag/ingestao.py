...
# Suprimir DeprecationWarning do langchain_community para uso normal do Chroma
with warnings.catch_warnings():
    warnings.simplefilter("ignore", DeprecationWarning)
    from langchain_community.vectorstores import Chroma

# Configurações
DOCS_DIR = Path(__file__).parent.parent.parent / "data" / "docs_regulatorios"
...
def carregar_documentos(docs_dir: Path = DOCS_DIR) -> list:
    """Carrega todos os arquivos .txt do diretório de documentos regulatórios."""
    docs = []
    for arquivo in docs_dir.glob("*.txt"):
        loader = TextLoader(str(arquivo), encoding="utf-8")
        docs_carregados = loader.load()
        # Adiciona metadado com o nome do documento (sem extensão)
...
def indexar_chroma(chunks: list, persist_dir: Path = CHROMA_DIR) -> Chroma:
    """Gera embeddings e indexa os chunks no Chroma."""
...
def carregar_vectorstore(persist_dir: Path = CHROMA_DIR) -> Chroma:
    """Carrega um vectorstore Chroma já existente."""
    embeddings = OllamaEmbeddings(model=EMBED_MODEL)
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", DeprecationWarning)
        return Chroma(
